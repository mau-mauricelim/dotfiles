...
- **Consistent location** — all worktrees live under `<repo>/.worktrees/`
...
**Path format:** `<repo-root>/.worktrees/<repo-name>.<sanitized-branch>`
...
▶ /home/user/projects/myapp/.worktrees/myapp.fe…  f4e5d6c  feature/login
  /home/user/projects/myapp/.worktrees/myapp.bu…  8a9b0c1  bugfix-123

  Managed in .worktrees/: 2  |  Repo root: /home/user/projects/myapp
```
...
Adds a `source` line pointing to `wt.init.sh`, which it expects at `../lib/wt.init.sh` relative to the `wt` binary (i.e. `~/.local/lib/wt.init.sh` with the recommended layout). Skips any file that already has it. Fails with the expected path if the file is not found.
...
```
<repo-root>/.worktrees/<repo-name>.<sanitized-branch>
```
...
|---|---|---|
| `myapp` | `main` | `myapp/.worktrees/myapp.main` |
| `myapp` | `feature/login` | `myapp/.worktrees/myapp.feature-login` |
| `myapp` | `bugfix-123` | `myapp/.worktrees/myapp.bugfix-123` |
| `api-server` | `chore/upgrade deps` | `api-server/.worktrees/api-server.chore-upgrade_deps` |

The `<repo-name>.` prefix keeps things identifiable when you open
`.worktrees/` in a file browser or editor sidebar, especially useful if you
...
```bash
wt() {
  # Locate the wt binary (../bin/wt relative to this file, then system paths)
  # Run it, capturing stdout to a temp file
  env WT_COLOR=1 "$_wt_bin" "$@" > "$_tmpout"

  # Extract cd signal
  _cd_path=$(grep "^__WT_CD__:" "$_tmpout" | sed 's/^__WT_CD__://')

  # Print everything except the signal line
  grep -v "^__WT_CD__:" "$_tmpout"

  # Actually cd in THIS shell
  [ -n "$_cd_path" ] && cd "$_cd_path"
}
```
...
Add `.worktrees/` to your repo's `.gitignore` so the worktree directories are
...
# → creates .worktrees/myapp.feature-payment-api
...
| Decision | Rationale |
|---|---|
| `.worktrees/` directory in repo root | Keeps everything self-contained. One `ls` shows all worktrees for the repo. Consistent across machines. |
