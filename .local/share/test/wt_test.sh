#!/usr/bin/env bash
# wt_test.sh — Comprehensive test suite for the wt Git Worktree Manager
# Usage:
#   bash wt_test.sh [path/to/wt]      # defaults to ./wt
#   WT_BIN=/usr/local/bin/wt bash wt_test.sh
set -euo pipefail

WT_BIN="${1:-${WT_BIN:-$(cd "$(dirname "$0")" && pwd)/wt}}"

# ── ANSI ──────────────────────────────────────────────────────────────────────
RED=$'\033[0;31m'; GREEN=$'\033[0;32m'; YELLOW=$'\033[1;33m'
CYAN=$'\033[0;36m'; BOLD=$'\033[1m'; DIM=$'\033[2m'; NC=$'\033[0m'

# ── State ─────────────────────────────────────────────────────────────────────
PASS=0; FAIL=0; SKIP=0
FAILED_TESTS=()

# ── Infrastructure ────────────────────────────────────────────────────────────

TMPDIR_ROOT=""
cleanup() {
  [[ -n "${TMPDIR_ROOT:-}" ]] && rm -rf "$TMPDIR_ROOT"
}
trap cleanup EXIT

make_repo() {
  # make_repo <dirname>  — creates a git repo with at least one commit
  local dir="$1"
  mkdir -p "$dir"
  git -C "$dir" init -q
  git -C "$dir" config user.email "test@test.com"
  git -C "$dir" config user.name  "Test"
  touch "$dir/README.md"
  git -C "$dir" add .
  git -C "$dir" commit -q -m "init"
}

WT() {
  # Run wt binary from within a directory, with color-compatible TERM.
  # Returns the stripped stdout (no __WT_CD__ lines).
  local dir="${_REPO:-}"
  TERM=xterm command "$WT_BIN" "$@" 2>&1 | grep -v '^__WT_CD__:'
}

WT_cd() {
  # Run wt and also extract the cd directive path (or empty string).
  TERM=xterm command "$WT_BIN" "$@" 2>&1 \
    | grep '^__WT_CD__:' | tail -1 | sed 's/^__WT_CD__://' || true
}

# ── Assertions ────────────────────────────────────────────────────────────────

_CURRENT_TEST=""

assert_eq() {
  local label="$1" got="$2" want="$3"
  if [[ "$got" == "$want" ]]; then
    return 0
  fi
  printf '    %bFAIL%b  %s\n' "$RED" "$NC" "$label"
  printf '      got:  %b%s%b\n' "$DIM" "$got" "$NC"
  printf '      want: %b%s%b\n' "$DIM" "$want" "$NC"
  return 1
}

assert_contains() {
  local label="$1" haystack="$2" needle="$3"
  if printf '%s' "$haystack" | grep -qF "$needle"; then
    return 0
  fi
  printf '    %bFAIL%b  %s\n' "$RED" "$NC" "$label"
  printf '      needle: %b%s%b\n' "$DIM" "$needle" "$NC"
  printf '      in:     %b%s%b\n' "$DIM" "$haystack" "$NC"
  return 1
}

assert_not_contains() {
  local label="$1" haystack="$2" needle="$3"
  if ! printf '%s' "$haystack" | grep -qF "$needle"; then
    return 0
  fi
  printf '    %bFAIL%b  %s\n' "$RED" "$NC" "$label"
  printf '      needle should NOT appear: %b%s%b\n' "$DIM" "$needle" "$NC"
  return 1
}

assert_dir_exists() {
  local label="$1" dir="$2"
  [[ -d "$dir" ]] && return 0
  printf '    %bFAIL%b  %s: directory not found: %s\n' "$RED" "$NC" "$label" "$dir"
  return 1
}

assert_dir_missing() {
  local label="$1" dir="$2"
  [[ ! -d "$dir" ]] && return 0
  printf '    %bFAIL%b  %s: directory should not exist: %s\n' "$RED" "$NC" "$label" "$dir"
  return 1
}

assert_exit_ok() {
  local label="$1"; shift
  local rc=0
  TERM=xterm command "$WT_BIN" "$@" &>/dev/null || rc=$?
  [[ $rc -eq 0 ]] && return 0
  printf '    %bFAIL%b  %s: expected exit 0, got %d\n' "$RED" "$NC" "$label" "$rc"
  return 1
}

assert_exit_fail() {
  local label="$1"; shift
  local rc=0
  TERM=xterm command "$WT_BIN" "$@" &>/dev/null || rc=$?
  [[ $rc -ne 0 ]] && return 0
  printf '    %bFAIL%b  %s: expected non-zero exit\n' "$RED" "$NC" "$label"
  return 1
}

# ── Test runner ───────────────────────────────────────────────────────────────

run_test() {
  local name="$1"; shift
  local fn="$1"; shift || true

  printf '  %b•%b %s … ' "$CYAN" "$NC" "$name"
  _CURRENT_TEST="$name"

  # Each test runs in its own temp repo
  TMPDIR_ROOT=$(mktemp -d)
  local repo="$TMPDIR_ROOT/repo"
  make_repo "$repo"
  export _REPO="$repo"

  local rc=0
  (
    cd "$repo"
    "$fn"
  ) || rc=$?

  rm -rf "$TMPDIR_ROOT"
  TMPDIR_ROOT=""

  if [[ $rc -eq 0 ]]; then
    printf '%bpass%b\n' "$GREEN" "$NC"
    (( PASS++ )) || true
  else
    printf '%bFAIL%b\n' "$RED" "$NC"
    (( FAIL++ )) || true
    FAILED_TESTS+=("$name")
  fi
}

skip_test() {
  local name="$1"
  printf '  %b○%b %s … %bskip%b\n' "$DIM" "$NC" "$name" "$YELLOW" "$NC"
  (( SKIP++ )) || true
}

section() { printf '\n%b%s%b\n' "$BOLD" "$1" "$NC"; }

# ═════════════════════════════════════════════════════════════════════════════
# TEST CASES
# ═════════════════════════════════════════════════════════════════════════════

# ── Sanitize ──────────────────────────────────────────────────────────────────
test_sanitize_slash() {
  local out
  out=$(TERM=xterm command "$WT_BIN" add "feature/login" 2>&1 || true)
  assert_dir_exists "creates repo.feature-login dir" \
    "$_REPO/.worktree/$(basename "$_REPO").feature-login"
}

test_sanitize_special_chars() {
  (
    cd "$_REPO"
    # Create branch with @ symbol in name
    git checkout -qb "fix/bug@1.0"
    git checkout -q main 2>/dev/null || git checkout -q master
    TERM=xterm command "$WT_BIN" add "fix/bug@1.0" 2>&1 || true
  )
  local repo_name
  repo_name=$(basename "$_REPO")
  assert_dir_exists "sanitizes @ in branch name" \
    "$_REPO/.worktree/${repo_name}.fix-bug-1.0"
}

test_sanitize_leading_dash() {
  # Branch  "feature/--double"  sanitizes dir name to "feature-double"
  # (consecutive dashes collapsed, no leading/trailing dashes)
  (cd "$_REPO" && TERM=xterm command "$WT_BIN" add "feature/--double" 2>&1 || true)
  local repo_name
  repo_name=$(basename "$_REPO")
  assert_dir_exists "collapses consecutive dashes" \
    "$_REPO/.worktree/${repo_name}.feature-double"
}

# ── wt add ────────────────────────────────────────────────────────────────────
test_add_new_branch() {
  cd "$_REPO"
  local out
  out=$(WT add feature/new-thing)
  assert_contains "output mentions worktree name" "$out" ".feature-new-thing"
  assert_dir_exists "worktree dir created" \
    "$_REPO/.worktree/$(basename "$_REPO").feature-new-thing"
}

test_add_existing_branch() {
  cd "$_REPO"
  git checkout -qb existing-branch
  git checkout -q main 2>/dev/null || git checkout -q master
  local out
  out=$(WT add existing-branch)
  assert_contains "output ok" "$out" "$(basename "$_REPO").existing-branch"
  # Verify the worktree uses the existing branch (not -b)
  local wt_path="$_REPO/.worktree/$(basename "$_REPO").existing-branch"
  assert_dir_exists "worktree dir exists" "$wt_path"
  local head_branch
  head_branch=$(git -C "$wt_path" branch --show-current 2>/dev/null || echo "")
  assert_eq "branch is existing-branch" "$head_branch" "existing-branch"
}

test_add_idempotent() {
  cd "$_REPO"
  WT add feature/idempotent > /dev/null
  local out
  out=$(WT add feature/idempotent)
  assert_contains "warns already exists" "$out" "already exists"
  # Ensure no nesting
  local repo_name
  repo_name=$(basename "$_REPO")
  assert_dir_missing "no nested worktree dir" \
    "$_REPO/.worktree/${repo_name}.feature-idempotent/.worktree"
}

test_add_emits_cd() {
  cd "$_REPO"
  local cd_target
  cd_target=$(WT_cd add feature/cd-test)
  local expected="$_REPO/.worktree/$(basename "$_REPO").feature-cd-test"
  assert_eq "cd directive points to new worktree" "$cd_target" "$expected"
}

test_add_auto_exclude() {
  cd "$_REPO"
  WT add feature/excl > /dev/null
  local exclude="$_REPO/.git/info/exclude"
  assert_contains ".worktree added to exclude" "$(cat "$exclude")" "/.worktree"
}

# ── wt rm ─────────────────────────────────────────────────────────────────────
test_rm_by_name() {
  cd "$_REPO"
  WT add feature/to-remove > /dev/null
  local wt_path="$_REPO/.worktree/$(basename "$_REPO").feature-to-remove"
  assert_dir_exists "setup: worktree exists" "$wt_path"
  WT rm feature/to-remove > /dev/null
  assert_dir_missing "worktree removed" "$wt_path"
}

test_rm_by_name_short() {
  # User can pass short name without prefix
  cd "$_REPO"
  WT add feature/short-rm > /dev/null
  WT rm feature/short-rm > /dev/null
  assert_dir_missing "removed by branch name" \
    "$_REPO/.worktree/$(basename "$_REPO").feature-short-rm"
}

test_rm_from_within() {
  cd "$_REPO"
  WT add feature/inner > /dev/null
  local wt_path="$_REPO/.worktree/$(basename "$_REPO").feature-inner"
  local cd_target
  # Simulate being inside the worktree
  cd_target=$(cd "$wt_path" && WT_cd rm)
  assert_eq "cd back to main repo" "$cd_target" "$_REPO"
  assert_dir_missing "worktree removed" "$wt_path"
}

test_rm_nonexistent_fails() {
  cd "$_REPO"
  assert_exit_fail "rm nonexistent exits non-zero" rm no-such-worktree
}

test_rm_from_main_without_name_fails() {
  cd "$_REPO"
  assert_exit_fail "rm without name from main fails" rm
}

# ── wt ls ─────────────────────────────────────────────────────────────────────
test_ls_shows_main() {
  cd "$_REPO"
  local out
  out=$(WT ls)
  assert_contains "ls shows [main]" "$out" "[main]"
}

test_ls_shows_worktrees() {
  cd "$_REPO"
  WT add feature/alpha > /dev/null
  WT add feature/beta  > /dev/null
  local out
  out=$(WT ls)
  local repo_name
  repo_name=$(basename "$_REPO")
  assert_contains "alpha in ls" "$out" "${repo_name}.feature-alpha"
  assert_contains "beta in ls"  "$out" "${repo_name}.feature-beta"
}

test_ls_alignment_long_name() {
  cd "$_REPO"
  local long="$(printf 'x%.0s' {1..60})"  # 60 x's
  WT add "$long" > /dev/null 2>&1 || true
  local out
  out=$(WT ls)
  # Should not crash; output should still contain [main]
  assert_contains "ls with long name still shows main" "$out" "[main]"
  # The long name should be truncated (ends with ...)
  assert_contains "long name truncated" "$out" "..."
}

test_ls_no_color_env() {
  cd "$_REPO"
  local out
  out=$(NO_COLOR=1 TERM=xterm command "$WT_BIN" ls 2>&1 | grep -v '^__WT_CD__:')
  # Should still work and show [main]
  assert_contains "ls works with NO_COLOR" "$out" "[main]"
  # Should not contain ANSI escape sequences
  if printf '%s' "$out" | grep -qP '\033\['; then
    printf '    FAIL  ls with NO_COLOR still has ANSI codes\n'
    return 1
  fi
}

# ── wt co ─────────────────────────────────────────────────────────────────────
test_co_existing() {
  cd "$_REPO"
  WT add feature/checkout-me > /dev/null
  local expected="$_REPO/.worktree/$(basename "$_REPO").feature-checkout-me"
  local cd_target
  cd_target=$(WT_cd co feature/checkout-me)
  assert_eq "co emits correct cd path" "$cd_target" "$expected"
}

test_co_nonexistent_fails() {
  cd "$_REPO"
  assert_exit_fail "co nonexistent fails" co no-such-branch
}

# ── wt mv ─────────────────────────────────────────────────────────────────────
test_mv_basic() {
  cd "$_REPO"
  WT add feature/old-name > /dev/null
  local repo_name
  repo_name=$(basename "$_REPO")
  local old_path="$_REPO/.worktree/${repo_name}.feature-old-name"
  local new_path="$_REPO/.worktree/${repo_name}.feature-new-name"

  WT mv feature/old-name feature/new-name > /dev/null
  assert_dir_missing "old path gone"    "$old_path"
  assert_dir_exists  "new path present" "$new_path"
}

test_mv_from_within_emits_cd() {
  cd "$_REPO"
  WT add feature/mv-from > /dev/null
  local repo_name
  repo_name=$(basename "$_REPO")
  local from_path="$_REPO/.worktree/${repo_name}.feature-mv-from"
  local new_path="$_REPO/.worktree/${repo_name}.feature-mv-to"

  local cd_target
  cd_target=$(cd "$from_path" && WT_cd mv feature/mv-from feature/mv-to)
  assert_eq "mv from within emits new path cd" "$cd_target" "$new_path"
}

test_mv_nonexistent_fails() {
  cd "$_REPO"
  assert_exit_fail "mv nonexistent source fails" mv no-such ghost
}

# ── wt prune ─────────────────────────────────────────────────────────────────
test_prune_runs() {
  cd "$_REPO"
  assert_exit_ok "prune exits 0" prune
}

test_prune_output() {
  cd "$_REPO"
  local out
  out=$(WT prune)
  assert_contains "prune output has 'pruning'" "$out" "pruning"
}

# ── wt nuke ──────────────────────────────────────────────────────────────────
test_nuke_force_removes_all() {
  cd "$_REPO"
  WT add feature/nuke1 > /dev/null
  WT add feature/nuke2 > /dev/null
  local repo_name
  repo_name=$(basename "$_REPO")

  WT nuke -f > /dev/null

  assert_dir_missing "nuke1 gone" "$_REPO/.worktree/${repo_name}.feature-nuke1"
  assert_dir_missing "nuke2 gone" "$_REPO/.worktree/${repo_name}.feature-nuke2"
}

test_nuke_deletes_branches() {
  cd "$_REPO"
  WT add feature/delbranch > /dev/null
  WT nuke -f > /dev/null

  local branches
  branches=$(git -C "$_REPO" branch --list "feature/delbranch")
  assert_eq "branch deleted after nuke" "$branches" ""
}

test_nuke_no_worktrees_message() {
  cd "$_REPO"
  local out
  out=$(WT nuke -f)
  assert_contains "message when nothing to nuke" "$out" "no linked worktrees"
}

test_nuke_from_within_emits_cd() {
  cd "$_REPO"
  WT add feature/was-here > /dev/null
  local repo_name
  repo_name=$(basename "$_REPO")
  local wt_path="$_REPO/.worktree/${repo_name}.feature-was-here"

  local cd_target
  cd_target=$(cd "$wt_path" && WT_cd nuke -f)
  assert_eq "nuke from worktree cds back to main" "$cd_target" "$_REPO"
}

# ── wt init ───────────────────────────────────────────────────────────────────
test_init_outputs_function() {
  local out
  out=$(TERM=xterm command "$WT_BIN" init)
  assert_contains "init outputs wt() function" "$out" "wt()"
  assert_contains "init uses command wt" "$out" "command wt"
  assert_contains "init handles cd directive" "$out" "__WT_CD__"
}

test_init_function_is_valid_shell() {
  # Should be eval-able in bash without errors
  local out
  out=$(TERM=xterm command "$WT_BIN" init)
  bash -c "$out" 2>&1
  # If we got here without error the function syntax is valid
  assert_eq "init function parses in bash" "ok" "ok"
}

test_init_function_is_valid_zsh() {
  if ! command -v zsh &>/dev/null; then
    printf '(zsh not available, skipping) '
    return 0
  fi
  local out
  out=$(TERM=xterm command "$WT_BIN" init)
  zsh -c "$out" 2>&1
  assert_eq "init function parses in zsh" "ok" "ok"
}

# ── Colors ────────────────────────────────────────────────────────────────────
test_colors_present_with_term() {
  # When TERM is set (non-dumb), output should contain ANSI codes
  cd "$(mktemp -d)"
  git init -q; git config user.email t@t.com; git config user.name T
  touch f; git add .; git commit -qm init
  local out
  out=$(TERM=xterm command "$WT_BIN" ls 2>&1 | grep -v '^__WT_CD__:')
  # Check for ESC [ sequence
  if printf '%s' "$out" | grep -qP '\033\['; then
    assert_eq "colors present with TERM=xterm" "yes" "yes"
  else
    printf '    FAIL  no ANSI codes in output even with TERM=xterm\n'
    return 1
  fi
}

test_colors_absent_with_no_color() {
  cd "$(mktemp -d)"
  git init -q; git config user.email t@t.com; git config user.name T
  touch f; git add .; git commit -qm init
  local out
  out=$(NO_COLOR=1 TERM=xterm command "$WT_BIN" ls 2>&1 | grep -v '^__WT_CD__:')
  if printf '%s' "$out" | grep -qP '\033\['; then
    printf '    FAIL  ANSI codes present despite NO_COLOR=1\n'
    return 1
  fi
  assert_eq "no colors with NO_COLOR=1" "ok" "ok"
}

test_colors_absent_with_dumb_term() {
  cd "$(mktemp -d)"
  git init -q; git config user.email t@t.com; git config user.name T
  touch f; git add .; git commit -qm init
  local out
  out=$(TERM=dumb command "$WT_BIN" ls 2>&1 | grep -v '^__WT_CD__:')
  if printf '%s' "$out" | grep -qP '\033\['; then
    printf '    FAIL  ANSI codes present with TERM=dumb\n'
    return 1
  fi
  assert_eq "no colors with TERM=dumb" "ok" "ok"
}

# ── Error handling ────────────────────────────────────────────────────────────
test_outside_git_repo() {
  local tmp
  tmp=$(mktemp -d)
  local rc=0
  (cd "$tmp" && TERM=xterm command "$WT_BIN" ls 2>&1) || rc=$?
  rm -rf "$tmp"
  [[ $rc -ne 0 ]] && return 0
  printf '    FAIL  should fail outside git repo\n'
  return 1
}

test_unknown_command_fails() {
  cd "$_REPO"
  assert_exit_fail "unknown command fails" totally-invalid-cmd
}

test_add_no_args_fails() {
  cd "$_REPO"
  assert_exit_fail "add without args fails" add
}

test_co_no_args_fails() {
  cd "$_REPO"
  assert_exit_fail "co without args fails" co
}

test_mv_missing_args_fails() {
  cd "$_REPO"
  assert_exit_fail "mv with one arg fails" mv only-one
}

# ── Version / help ────────────────────────────────────────────────────────────
test_help_exits_ok() {
  assert_exit_ok "help exits 0" help
}

test_version_exits_ok() {
  assert_exit_ok "version exits 0" version
}

test_version_contains_number() {
  local out
  out=$(TERM=xterm command "$WT_BIN" version)
  if printf '%s' "$out" | grep -qE '[0-9]+\.[0-9]+'; then
    return 0
  fi
  printf '    FAIL  version output does not contain a version number\n'
  return 1
}

# ═════════════════════════════════════════════════════════════════════════════
# RUN ALL TESTS
# ═════════════════════════════════════════════════════════════════════════════

[[ -x "$WT_BIN" ]] || { printf '%berror:%b %s is not executable\n' "$RED" "$NC" "$WT_BIN"; exit 1; }

printf '\n%bwt test suite%b  (%b%s%b)\n' "$BOLD" "$NC" "$DIM" "$WT_BIN" "$NC"

section "Sanitize"
run_test "slash in branch name"             test_sanitize_slash
run_test "special chars (@) in branch name" test_sanitize_special_chars
run_test "leading slash/dash stripped"      test_sanitize_leading_dash

section "wt add"
run_test "add new branch"                   test_add_new_branch
run_test "add existing branch (no -b)"      test_add_existing_branch
run_test "add idempotent (no nesting)"      test_add_idempotent
run_test "add emits cd directive"           test_add_emits_cd
run_test "add updates .git/info/exclude"    test_add_auto_exclude

section "wt rm"
run_test "rm by branch name"                test_rm_by_name
run_test "rm by short name"                 test_rm_by_name_short
run_test "rm from within worktree"          test_rm_from_within
run_test "rm nonexistent exits non-zero"    test_rm_nonexistent_fails
run_test "rm from main without name fails"  test_rm_from_main_without_name_fails

section "wt ls"
run_test "ls shows [main]"                  test_ls_shows_main
run_test "ls shows linked worktrees"        test_ls_shows_worktrees
run_test "ls handles very long names"       test_ls_alignment_long_name
run_test "ls respects NO_COLOR"             test_ls_no_color_env

section "wt co"
run_test "co existing worktree"             test_co_existing
run_test "co nonexistent exits non-zero"    test_co_nonexistent_fails

section "wt mv"
run_test "mv renames worktree dir"          test_mv_basic
run_test "mv from within emits cd"          test_mv_from_within_emits_cd
run_test "mv nonexistent source fails"      test_mv_nonexistent_fails

section "wt prune"
run_test "prune exits 0"                    test_prune_runs
run_test "prune has expected output"        test_prune_output

section "wt nuke"
run_test "nuke -f removes all worktrees"    test_nuke_force_removes_all
run_test "nuke -f deletes branches"         test_nuke_deletes_branches
run_test "nuke with nothing to remove"      test_nuke_no_worktrees_message
run_test "nuke from worktree cds to main"   test_nuke_from_within_emits_cd

section "wt init"
run_test "init outputs shell function"      test_init_outputs_function
run_test "init function valid bash syntax"  test_init_function_is_valid_shell
run_test "init function valid zsh syntax"   test_init_function_is_valid_zsh

section "Colors"
run_test "colors present with TERM=xterm"   test_colors_present_with_term
run_test "colors absent with NO_COLOR=1"    test_colors_absent_with_no_color
run_test "colors absent with TERM=dumb"     test_colors_absent_with_dumb_term

section "Error handling"
run_test "fails outside git repo"           test_outside_git_repo
run_test "unknown command fails"            test_unknown_command_fails
run_test "add without args fails"           test_add_no_args_fails
run_test "co without args fails"            test_co_no_args_fails
run_test "mv with one arg fails"            test_mv_missing_args_fails

section "Version / Help"
run_test "help exits 0"                     test_help_exits_ok
run_test "version exits 0"                  test_version_exits_ok
run_test "version contains number"          test_version_contains_number

# ── Summary ───────────────────────────────────────────────────────────────────

total=$(( PASS + FAIL + SKIP ))
printf '\n%b────────────────────────────────────────%b\n' "$DIM" "$NC"
printf ' %b%d passed%b  ' "$GREEN" "$PASS" "$NC"
if [[ $FAIL -gt 0 ]]; then
  printf '%b%d failed%b  ' "$RED" "$FAIL" "$NC"
fi
if [[ $SKIP -gt 0 ]]; then
  printf '%b%d skipped%b  ' "$YELLOW" "$SKIP" "$NC"
fi
printf 'of %d total\n' "$total"
printf '%b────────────────────────────────────────%b\n\n' "$DIM" "$NC"

if [[ $FAIL -gt 0 ]]; then
  printf '%bFailed tests:%b\n' "$RED" "$NC"
  for t in "${FAILED_TESTS[@]}"; do
    printf '  • %s\n' "$t"
  done
  printf '\n'
  exit 1
fi

exit 0
