...
.tree.i.branch:{[params]
    ...
    raze{[params;entry;isLast]
        ...
        typeD,$[params[`maxDepth]&typeD;.tree.i.branch[params];()]
        }[params]'[entries;entries=last entries]
    };
/ If function is called with [], path defaults to current directory: `:.
tree :.tree.tree :{[path] .tree.i.tree[;10;1b;0b]$[(::)~path;`:.;path]};
treea:.tree.treea:{[path] .tree.i.tree[;10;1b;1b]$[(::)~path;`:.;path]};
