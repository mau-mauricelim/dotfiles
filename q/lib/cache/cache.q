...
cacheFn:.cache.cacheFn:{[fnName;fn]
    ...
    .cache.cacheInit fnName;
    / General cached function
    cachedFn:{[fnName;fn;params]
        / Retrieve result from cache with serialized (-8!) input params
        if[not(::)~res:.cache.cache[fnName]byte:-8!params;:res];
        / Compute the result and add it to the cache
        :.cache.cache[fnName;byte]:fn . params}[fnName;fn];
...
cacheInit:.cache.cacheInit:{[fnName] .cache.cache[fnName]:(`g#enlist 0#0x0)!enlist{value}};
