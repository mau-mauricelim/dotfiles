...
main() {
    ...
    WIN_OS=w$BIT
    LIN_OS=l$BIT
    command -v rg >/dev/null && local GREP=rg || local GREP=grep
    ...
    for q_ver_path in $(get_q_ver_path "$LIN_Q_HOME" "$LIN_OS"); do
        local q_ver_installed=$(get_q_ver "$q_ver_path")
...
alias_setup() {
    local stripped_version=$(echo "$1" | tr -d '.') # 4.1 becomes 41
...
wsl_kdb_setup() {
    ...
    if [ -d "$WIN_Q_HOME" ]; then
        ...
        for win_q_ver_path in $(get_q_ver_path "$WIN_Q_HOME" "$LIN_OS"); do
            local win_q_ver_avail=$(get_q_ver "$win_q_ver_path")
            for q_ver_path in $(get_q_ver_path "$LIN_Q_HOME" "$LIN_OS"); do
                local q_ver_installed=$(get_q_ver "$q_ver_path")
...
run_q() {
    ...
    export PATH=$(echo "$PATH" | sed "s:$LIN_Q_HOME/.*/$LIN_OS:$QHOME/$LIN_OS:g")
    QCMD="q ${@:2}"
